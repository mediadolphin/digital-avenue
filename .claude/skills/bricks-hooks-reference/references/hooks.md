...
Search the relevant family and verify the signature against the installed version.
...
| Hook | Kind | Purpose |
|---|---|---|
| `bricks/element/render` | filter | Boolean render gate: return false to omit an element |
| `bricks/frontend/render_element` | filter | Modify rendered element HTML; receives HTML and the element instance |
...
- Treat every query hook as if it passed a Query object. `bricks/posts/query_vars` supplies args, settings, element ID and element name; scope using its element-ID argument.
